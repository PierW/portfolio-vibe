...
Research me from these sources: [paste LinkedIn, YouTube, X, GitHub, personal site links] **or** get data from fetch website portfolio.themillennials.it.
...
Create a single page aspplication in vue.js 3 composition api (Tailwind v4) styled like a terminal. **No animations** — everything renders on first paint.

**Palette (use these exact hex values, wired as CSS variables and exposed via `@theme inline` so they're available as Tailwind utilities):**
...
**Font:** monospace throughout. set it as the `body` font-family.
